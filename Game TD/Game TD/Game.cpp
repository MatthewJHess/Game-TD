...
Game::Game(const std::shared_ptr<sf::RenderWindow>& window) : player(spells, textureManager, 50.0f, 50.0f)
{
...
Game::~Game()
{
    // Smart pointers automatically clean up memory, no need to delete
}
...
void Game::handleTurretPlacement() {
    if (placingTurret) {
        ...
        if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
            ...
            this->turrets.emplace_back(mousePos.x, mousePos.y, textureManager);
